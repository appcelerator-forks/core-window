...
var App = {
	...
	openScreen: function(_controller, _controllerArguments) {
		...
		if(!_controllerArguments) {
			_controllerArguments = {};
		}
		...
		// Push the wrapper down 47dp to account for navigation bar
		...
		controller.window = Ti.UI.createWindow({
			...
			barColor: "#3D3D3C",
			translucent: false,
			extendEdges: OS_IOS ? [Ti.UI.EXTEND_EDGE_NONE] : null,
			navTintColor: "#FFF",
			tintColor: "#FFF",
			color: "#FFF",
			top: OS_IOS ? (App.versionMajor < 7 ? 0 : 20) : 0,
			windowSoftInputMode: OS_ANDROID ? Ti.UI.Android.SOFT_INPUT_ADJUST_RESIZE : null,
			softKeyboardOnFocus: OS_ANDROID ? Ti.UI.Android.SOFT_KEYBOARD_HIDE_ON_FOCUS : null,
			exitOnClose: OS_ANDROID ? (_controllerArguments.goBack ? false : true ) : false
		});
		...
		// Set properties of the navigation bar
		...
		if(_controllerArguments && _controllerArguments.goBack) {
			controller.navigation.showBack(function(_event) {
				if(OS_IOS) {
